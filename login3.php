		...
		if(isset($_POST["username"]) && isset($_POST["password"])) {
			...
			if ($stmt = $mysqli->prepare("select uid, username, password, role from customer where username = ? and password = ?")) {
				...
				if(isset($_SESSION["uid"])){
					...
					$diff;
					if ($stmt = $mysqli->prepare("select regTime from customer where where uid=?")) {
						$stmt->bind_param("i",$_SESSION["uid"]);
						$stmt->execute();
						$stmt->bind_result($lasttime);
						//if there is a match set session variables and send user to homepage
						if ($stmt->fetch()) {
							$diff=ceil(($nowtime-$lasttime)/86400)/30;
						}
						$stmt->close();
					}
					...
					if($diff>4){
						...
						if ($stmt = $mysqli->prepare("update users set trust=$diff where uid=? and where trust<4")){
							$stmt->bind_param("i",$_SESSION["uid"]);
							$stmt->execute();
							$stmt->close();
						}
